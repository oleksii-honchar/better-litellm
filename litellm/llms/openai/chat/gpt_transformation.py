...
class OpenAIGPTConfig(BaseLLMModelInfo, BaseConfig):
    ...
    def _check_and_fix_if_content_is_tool_call(
        ...
        5. JSON in <tool_call> tags (opencode-style or OpenAI-style tool calls)
        6. Pure JSON format (backward compatibility)
        """
        import json
        ...
        # 1) Try Qwen XML format in ```_ tags
        try:
        ...
        # 2) Try JSON format inside ```_ tags
        try:
        ...
        try:
            ...
            if cdata_xml:
                tool_call = self._parse_qwen_xml_to_tool_call(cdata_xml)
                if tool_call and tool_call.function.name in tool_call_names:
                    return tool_call
        except Exception:
            pass

        # 5) Try JSON format in <tool_call> tags (opencode-style or OpenAI-style)
        try:
            json_wrapper = self._extract_json_from_tool_call_wrapper_tags(content)
            if json_wrapper:
                tool_call = self._parse_opencode_json_to_tool_call(json_wrapper)
                if tool_call and tool_call.function.name in tool_call_names:
                    return tool_call
        except Exception:
            pass

        # 6) Fall back to pure JSON format (backward compatibility)
        try:
    ...
    def _extract_xml_from_tool_call_tags(content: str) -> Optional[str]:
        import re
        for match in re.finditer(r'```_\s*(.*?)\s*_```', content, re.DOTALL):
    ...
    def _extract_json_from_tool_call_tags(content: str) -> Optional[str]:
        import re
        match = re.search(r'```_\s*(.*?)\s*_```', content, re.DOTALL)
        if match:
    ...
    @staticmethod
    def _extract_xml_from_cdata_tags(content: str) -> Optional[str]:
        """Extract Qwen XML tool call from <![CDATA[...]]> tags.

        Anthropic-style CDATA-wrapped Qwen XML:
        <![CDATA[<function=grep><parameter=pattern>...</parameter></function>]]>
        """
        import re
        for match in re.finditer(
            r'<!\[CDATA\[(<function=\S+?>.*?</function>)\]\]>', content, re.DOTALL
        ):
            xml_content = match.group(1).strip()
            if xml_content.startswith("<function="):
                return xml_content
    ...
    @staticmethod
    def _extract_json_from_tool_call_wrapper_tags(content: str) -> Optional[str]:
        """Extract JSON tool call from <tool_call> wrapper tags.

        opencode format with <tool_call> wrapper:
        <tool_call>
    ...
    def _parse_opencode_json_to_tool_call(json_content: str) -> Optional[ChatCompletionMessageToolCall]:
        ...
        try:
            ...
            if not tool_name:
                return None
            # If OpenAI-style with "arguments" key
    ...
    def _extract_tool_calls_from_llamacpp_error(
        ...
        - <tool_call>...</tool_call> tags (Qwen format wrapper — XML or JSON)
        ...
        # Try Qwen XML format in <![CDATA[...]]> tags
        for cdata_match in re.finditer(
            r'<!\[CDATA\[(<function=\S+?>.*?</function>)\]\]>', raw_output, re.DOTALL
        ):
        ...
        # Try JSON format in <tool_call> tags (opencode-style or OpenAI-style tool calls)
        for json_match in re.finditer(
            r'<tool_call[^>]*>\s*(\{.*?\})\s*</tool_call>',
            raw_output,
            re.DOTALL,
            ...
            try:
                ...
                if not tool_name:
                    continue
                # Build arguments from remaining keys
                arguments = {
                    k: v for k, v in parsed.items()
                    if k not in ("type", "name")
                }
...
class OpenAIChatCompletionStreamingHandler(BaseModelResponseIterator):
    ...
    def _extract_tool_calls_from_buffer(self):
        ...
        # Qwen XML pattern in ```_ tags
        xml_pattern = re.compile(chr(96)*3 + r'_\s*(<function=\S+?>.*?</function>)\s*_' + chr(96)*3, re.DOTALL)
        ...
        # JSON pattern in ```_ tags
        json_pattern = re.compile(chr(96)*3 + r'_\s*(\{.*?\})\s*_' + chr(96)*3, re.DOTALL)
        ...
        # Qwen XML pattern in <![CDATA[...]]> tags
        cdata_pattern = re.compile(r'<!\[CDATA\[(<function=\S+?>.*?</function>)\]\]>', re.DOTALL)
        for match in reversed(list(cdata_pattern.finditer(remaining))):
            xml_content = match.group(1)
            tool_call = OpenAIGPTConfig._parse_qwen_xml_to_tool_call(xml_content)
            if tool_call:
                tc_with_id = ChatCompletionMessageToolCall(
        ...
        # JSON pattern in <tool_call> tags (opencode-style or OpenAI-style tool calls)
        json_wrapper_pattern = re.compile(r'<tool_call[^>]*>\s*(\{.*?\})\s*</tool_call>', re.DOTALL)
        for match in reversed(list(json_wrapper_pattern.finditer(remaining))):
            json_content = match.group(1)
            try:
                parsed = json.loads(json_content)
                tool_name = parsed.get("type") or parsed.get("name")
